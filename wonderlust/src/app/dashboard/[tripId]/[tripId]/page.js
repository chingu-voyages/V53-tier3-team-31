// In tripdetails/[tripId].js
"use client";
import { useState, useEffect } from "react";
import { useRouter } from "next/router"; // Import useRouter for dynamic URL fetching
...
import Locations from "@/src/components/TripDetails/Locations";
...
export default function TripDetail() {
  ...
  const { query } = useRouter(); // Get tripId from the URL

  useEffect(() => {
    const tripId = query.tripId; // Get tripId from the URL params

    // Static Data (for now)
    if (tripId === "679e6e05189dd60ff447e7fd") {
      setTrip({
      ...
      >
        ...
        <div className="p-6 shadow-md ">
          <Timeline>
            ...
            <Locations setSidebarOpen={setSidebarOpen} />
          </Timeline>
        </div>
      </div>
